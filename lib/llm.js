...
function resolveOpenClawLLM() {
  ...
  if (typeof defaultModel === 'object' && defaultModel.primary) {
    defaultModel = defaultModel.primary;
  }

  if (typeof defaultModel !== 'string') return null;
...
export async function callLLM(messages, options = {}) {
  ...
  const apiType = process.env.OPENAI_API_TYPE || llm?.apiType || 'openai';
  const model = options.model || process.env.OPENAI_MODEL || llm?.modelId || 'gpt-4o';
